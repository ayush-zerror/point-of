...
const GridButton = ({ title, onClick, color = "#c0bfbf", className, textClassName }) => {
  ...
  const inner = (
    <>
      {/* Icon (fades out on hover) + ring (shows on hover) */}
      <span
        style={iconStyle}
      ...
      >
        <span
          className={`absolute w-2.5 h-2.5 rounded-full border opacity-0 transition-opacity duration-300 group-hover:opacity-100 ${
        ...
        <svg
          ...
          className="relative w-3 h-3 transition-opacity duration-300 group-hover:opacity-0"
        >
          <g>
            <path
